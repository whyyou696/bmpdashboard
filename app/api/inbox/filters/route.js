...
export async function GET() {
  try {
    ...
    const products = await pool.request().query("SELECT DISTINCT kode_produk FROM transaksi WITH (NOLOCK) WHERE kode_reseller LIKE 'BEST%' AND kode_produk IS NOT NULL AND kode_produk != '' ORDER BY kode_produk");
    const terminals = await pool.request().query("SELECT DISTINCT kode_terminal FROM inbox WITH (NOLOCK) WHERE kode_terminal IS NOT NULL ORDER BY kode_terminal");
    const serviceCenters = await pool.request().query("SELECT DISTINCT service_center FROM inbox WITH (NOLOCK) WHERE service_center IS NOT NULL AND service_center != '' ORDER BY service_center");
